    ...
    <main className="min-h-screen bg-surface-page px-4 py-5 text-foreground sm:px-6 lg:px-8">
      <form action={formAction} className="mx-auto flex max-w-6xl flex-col gap-5">
        <div className="flex flex-col gap-1">
          ...
          <header className="rounded-xl border bg-surface-card p-5 shadow-2">
        ...
        <section className="grid items-start gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Privy login status, wallet balance, and linked account context.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex flex-wrap items-center gap-3 rounded-lg border bg-surface-subtle p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-text-3">{privy.user?.id ?? "Login to create or connect a wallet."}</p>
                </div>
                {!privy.authenticated ? (
                  <Button type="button" disabled={!privy.ready} onClick={() => privy.login({ loginMethods: ["email", "twitter", "github"] })}>
                    Login
                  </Button>
                ) : (
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-lg border bg-surface-subtle p-3 sm:col-span-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Available endpoints and the actual Privy-wallet request action.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                  ...
                  <label key={endpoint.path} className={endpoint.path === endpointInput ? "rounded-lg border border-primary/30 bg-mesh-blue-dim p-3" : "rounded-lg border bg-surface-subtle p-3"}>
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-sm text-text-3"><span className="mono">{endpoint.method} {endpoint.path}</span> - {endpoint.detail}</p>
                      </div>
                    </div>
                  </label>
                ))}
              ...
              <div className="rounded-lg border bg-surface-subtle p-3 text-sm text-text-2">
                <p><span className="text-text-mute">Endpoint:</span> {selectedEndpoint.method} {endpointInput}</p>
                <p><span className="text-text-mute">x-agent-wallet:</span> {shortAddress(privyWallet)}</p>
                <p><span className="text-text-mute">x-agent-budget:</span> {budgetInput}</p>
              </div>
              ...
              <p className={state.status === "error" ? "rounded-lg border border-danger/20 bg-danger-soft p-3 text-sm text-danger" : "rounded-lg border border-primary/20 bg-mesh-blue-dim p-3 text-sm text-text-2"}>
                {state.message}
              </p>
            </CardContent>
          </Card>
